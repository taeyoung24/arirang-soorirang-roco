...
export default function AnswerButton({ parts, status = 'Default', onClick }) {
  ...
  let highlightBorderClass = styles.borderDefault;
  let iconSvg = null;

  switch(status) {
    case 'Disable':
      bgClass = styles.bgSoftdark;
      opacityClass = styles.opacity60;
      iconSvg = (
    ...
    case 'Selected-Incorrect':
      ...
      break;
    case 'Selected':
      bgClass = styles.bgSooriPrimary;
      textColorClass = styles.textWhite;
      highlightBorderClass = styles.borderWhite;
      iconSvg = (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className={styles.textWrapper}>
          {parts.map((part, index) => {
            if (part.type === 'highlight') {
              return (
                <div key={index} className={`${styles.highlightBlock} ${highlightBorderClass}`}>
                  <span className={`${styles.highlightText} ${textColorClass}`}>{part.text}</span>
                </div>
              );
            }
            return (
              <span key={index} className={`${styles.normalText} ${textColorClass}`}>{part.text}</span>
            );
          })}
        </div>
      </div>
