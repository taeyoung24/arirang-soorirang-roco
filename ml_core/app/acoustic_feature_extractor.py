...
class StandardAcousticFeatureExtractor:
    ...
    def extract_prosody(
        ...
        if forced_summary is not None:
            forced_summary.utterance_f0_mean_hz = f0_mean
            forced_summary.utterance_f0_range_semitones = f0_range
            forced_summary.phrase_final_f0_slope = phrase_final_slope
    ...
    def _pause_level(user_pause_ms: int, duration_delta_ms: int) -> str | None:
        if user_pause_ms >= 1200 and duration_delta_ms >= 800:
            return "high"
        if user_pause_ms >= 800 and duration_delta_ms >= 450:
            return "medium"
        return None
